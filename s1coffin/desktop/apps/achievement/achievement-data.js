...
const ALL_ACHIEVEMENTS = [
    ...
    },
    {
        ...
        icon: 'signature',
        name: '线索笔录员',
        condition: '使用记事本写下第一条笔记',
        desc: '记录关键线索，调查脉络清晰分明',
        check: () => {
    ...
    },
    {
        ...
        icon: 'magnifier',
        name: '捕柴达人',
        condition: '找到藏在网页里的“柴不知”',
        desc: '正经的网页里混进了奇怪的名字',
        check: () => {
            const t = achRead('notebook_triggers');
            return !!(t.chai_found_3q && t.chai_found_job);
        }
    },
    {
        id: 'explore_projects',
    ...
    },
    {
        ...
        icon: 'news',
        name: '公告猎人',
        condition: '阅读完员工系统的所有通知',
        desc: '通知、警告一条不落，内部动态被你尽数掌握',
        check: () => {
            const read = achRead('intranet_state').noticesRead || [];
            const all = [
                'notice-south-africa-close',
                'notice-ding-death',
                'notice-cloud-archive',
                'notice-profile-confirm',
                'security-warning',
                'upgrade'
            ];
            return all.every(id => read.includes(id));
        }
    },
    {
        id: 'explore_scholar',
